...
test('gravações, filtros e erros informam resultados sem sucesso falso',async()=>{
 ...
 t.view('materiais');assert.match(await t.click('open-resource'),/aberto/);t.view('inicio');assert.match(await t.click('logout'),/saiu/);assert.equal(await t.click('dismiss-notice'),'');
});
